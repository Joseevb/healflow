...
import type { ApiProblemDetail, ProvisionUserRequest, ValidationProblemDetail } from "@/client";
import type { ApiKeyConfig } from "@/lib/api-key.config";
import { provisionUser } from "@/client";
import { auth } from "@/lib/auth";

export class UserRegistrationError extends TaggedError("UserRegistrationError")<{
  message?: string;
}> {}
        ...
        try: async () => {
          ...
          switch (errorRes?.status) {
            case 400:
              ...
              break;
            case 401:
              errorMessage = (errorRes as ApiProblemDetail).detail || "Unauthorized";
              break;
            case 403:
              errorMessage = (errorRes as ApiProblemDetail).detail || "Forbidden";
              break;
          }
