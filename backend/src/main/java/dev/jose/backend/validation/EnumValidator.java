...
public class EnumValidator implements ConstraintValidator<ValidEnum, Object> {
  ...
  public void initialize(ValidEnum annotation) {
    ...
    acceptedValues = Arrays.stream(enumConstants).map(Enum::name).collect(Collectors.joining(", "));
  }
  ...
  public boolean isValid(Object value, ConstraintValidatorContext context) {
    ...
    if (!isValid) {
      ...
      context
          .buildConstraintViolationWithTemplate("Invalid value. Accepted values: " + acceptedValues)
