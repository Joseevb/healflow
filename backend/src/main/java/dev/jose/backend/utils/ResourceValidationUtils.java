...
import static dev.jose.backend.api.exceptions.GlobalExceptionHandler.extractErrorMessages;

import dev.jose.backend.api.exceptions.ResourceAlreadyExistsException;
import dev.jose.backend.api.exceptions.ValidationException;
...
public class ResourceValidationUtils {
   ...
   *
   * @param lookupFunction A function representing the repository method to find a user by a unique
   *     field (e.g., userRepository::findByEmail).
   ...
   * @throws ResourceAlreadyExistsException if a user with the specified field value already exists.
   */
  ...
  /**
   * Validates that a resource does not already exist based on a given unique field and its value,
   * reactively. If a resource is found, a {@link ResourceAlreadyExistsException} is emitted. This
   * method is designed to be called before creating a new resource or updating a unique field.
   *
   * @param lookupFunction A function representing the repository method to find a user by a unique
   *     field (e.g., userRepository::findByEmail).
   * @param fieldValue The value of the unique field to check against (e.g., the email address).
   ...
   * @param <T> The type of the field value (e.g., String).
   * @param <E> The type of the entity being searched (e.g., UserEntity).
   * @return A {@link Mono<Void>} that completes if the resource does not exist, or emits a {@link
   *     ResourceAlreadyExistsException} if it does.
   */
  public static <T, E> Mono<Void> validateResourceDoesNotExistReactive(
      Function<T, Mono<E>> lookupFunction, T fieldValue, String fieldName, String resourceName) {
  ...
  public static <T> Mono<Void> validateResource(T resource, Validator validator) {
    var violations = validator.validate(resource);
    if (!violations.isEmpty()) {
      var errorMessages = extractErrorMessages(violations);
