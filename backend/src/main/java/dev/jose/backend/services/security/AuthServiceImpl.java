...
public class AuthServiceImpl implements AuthService {
    ...
    return authenticationManager
        ...
        .flatMap(
            authentication -> {
              ...
              final String jwt =
                  jwtEncoder.encode(JwtEncoderParameters.from(jwtClaims)).getTokenValue();
  ...
  @Override
  @Transactional
  public Mono<UserResponseDto> register(RegisterUserRequestDto register) {
    return Mono.just(userMapper.toDto(register))
        .flatMap(userService::createUser)
        .flatMap(
            createdUserDto ->
                userService
                    .getUserEntityById(createdUserDto.id())
                    .flatMap(
                        userEntity ->
                            userVerificationService
                                .createAndSendVerificationToken(userEntity)
              ...
              if (authentication == null
                ...
                return Mono.error(
                    new UnauthorizedOperationException(
                        "User not authenticated or principal type" + " incorrect"));
              }
                  ...
                  .flatMap(
                      isValid -> {
                        if (!isValid) {
                          ...
                          return Mono.error(new UnauthorizedOperationException("Invalid token"));
                        }
